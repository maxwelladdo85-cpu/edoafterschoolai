...
CREATE OR REPLACE FUNCTION public.send_teacher_announcement(p_class_level text, p_title text, p_message text)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  IF NOT (
    ...
    OR EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role::text = 'scripter')
  ) THEN
